...
{
	struct tm _FAR* _lp = localtime( (time_t *)&ct_ );
...
{
	//usually, we don't use DST format
	CORE_ASSERT( lptm_->tm_isdst == 0 );

	return (CORETIME)mktime( lptm_ );
}
