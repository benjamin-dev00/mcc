...
ACRET ReadFileSR( LPCOBJPRIV lpCobjPrivAll_ )
{
	...
	double			_mG[pp][mm];
#endif
	READALLPARAMETER( lpCobjPrivAll_, MDMCIRVD_mSR,LPCOBJ,_lpCobj_SR );	
	READALLPARAMETER( lpCobjPrivAll_, MDMCIRVD_mDSR,LPCOBJ,_lpCobj_DSR );	
	...
	for( _i = 0; _i < lpCobjPrivAll_->p; _i++ )
	{
		...
		for( _j = 0; _j < lpCobjPrivAll_->m; _j++ )
		{
			...
			sprintf( _FileName, "%s%s%s%s%s%s", LocalDirectory, "mccstp\\CV", _sz_i, "MV", _sz_j, ".stp" );
...
//				return ACRET_FALSE;
		...
		if( lpCobjPrivAll_->nd )
		{
			for( _k = 0; _k < lpCobjPrivAll_->nd; _k++ )		
			{
				...
				sprintf( _FileName, "%s%s%s%s%s%s", LocalDirectory, "mccstp\\CV", _sz_i, "DV", _sz_k, ".stp" );
				if ((_fin=fopen( _FileName, "r"))==NULL)
				{
//					MessageBox( NULL, "cannot open stp file\n", "", MB_OK );
//					return ACRET_FALSE;
...
#ifdef __debug
	sprintf( _FileName, "%s%s", LocalDirectory, "tfg.txt" );
	if ((_fin=fopen( _FileName, "w"))==NULL)
	{
//		MessageBox( NULL, "cannot open stp file\n", "", MB_OK );
		return ACRET_FALSE;
	}
	for ( _i = 0; _i < lpCobjPrivAll_->p; _i++)
	{
		for ( _j = 0; _j < lpCobjPrivAll_->m; _j++)
		{
			_mG[_i][_j] = DIMMGET( _lpDimSR, _i, _j, (lpCobjPrivAll_->N-1) );
